...
import jobs4u.core.backoffice.application.VerifyApplicationRequirementsController;
...
@Component
public class VerifyApplicationRequirementsUI extends AbstractUI {
    @Autowired
    private VerifyApplicationRequirementsController controller;
    @Override
    ...
    private JobOpening chooseJobOpening(){
        List<JobOpening> jobOpenings = controller.getOpeningsInInterviewPhase();
        if(jobOpenings.isEmpty()) {
            System.out.println("You have no registered Job Openings in the Screening phase.");
            return null;
        }
    ...
    @Override
    public String headline() {return "Execute the requirements for Job Applications";}
}
